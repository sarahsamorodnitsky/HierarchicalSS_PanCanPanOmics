...
# by every cancer type. Assumes survival is log-normally distributed.
# This model implements a spike-and-slab prior on the effect of each 
# principal component on survival. 
# NOTE**: In this model, the intercept is outside the spike-and-slab framework
# and has its on prior. Also in this version of the model I create a priors
# argument to control the priors I use outside the function.
# I have also created a pi_generation argument to dictate how the 
# prior inclusion probability is generated. 
...
HierarchicalLogNormalSpikeSlab = function(Covariates, Survival, Censored, 
  ...
  # pi_generation (character): Describes how to generate prior inclusion probability pi with these argument values: "shared_across_cancers", "fixed_at_0.5", "fixed_at_1.0", "shared_across_betas", "fixed_at_0.0" (either normal, fixed at 0.5, 1.0, shared across all covariates, or no covariates are included) 
  # progress (character): specifies how progress in model running should be printed: normal if EnvMisc progress bar, model_comparison if cat to an outside text file, simulation if prints when model completes
  # condition (character): specifies what condition this model is running data under for the progress bar for simulations
  ...
  Beta_Posterior_Helper = function(x_i, y_i, gamma_ij, sigma2_i, lambda2_i, beta_tilde_i) {
    ...
    # If there is only one covariate in the model
    if (p == 1) { 
      VarCovarMatrix = 1/Variance # Calling diag(number less than 1) returns a 0x0 matrix
      
    } else { # If there are multiple covariates in the model
      VarCovarMatrix = diag(1/Variance)
    }
    
    # Mean vector
    beta_tilde_after_adding_0s = beta_tilde_i * gamma_ij
    
    B = solve((1/sigma2_i)*t(x_i)%*%x_i + VarCovarMatrix, tol = 1e-35) # if (length(lambda2_i) == 1) solve((1/sigma2_i)*t(x_i)%*%x_i + (1/lambda2_i)) else 
  ...
  for (k in 1:n_cancer) {
    ...
    Y[[k]] = log(Y[[k]]) # for log-normal model
  }
  
  for (i in 1:iters) {
    # progress bar I use when just running the model
    if (progress == "normal") {
      svMisc::progress(i/(iters/100))
    }
    # progress bar I use when I am running things in parallel for model comparison
    else if (progress == "model_comparison") {
      if (i %% 10000 == 0) cat(paste("Model", pi_generation, "with", p, "covariates is at iteration", i), "\n")
    }
    # else if (progress == "simulation") {
    #   if (i %% 200 == 0) cat(paste("Model ", pi_generation, "is at iteration", i), "\n")
    # }
    ...
    if (pi_generation == "shared_across_cancers") {
      ...
      pi[i+1,] = rep(0.5, p) # fix prior inclusion probability at 0.5
    } else if (pi_generation == "fixed_at_1.0") {
      pi[i+1,] = rep(1.0, p) # fix prior inclusion probability at 0.5
    } else if (pi_generation == "shared_across_betas") { # store the same value for every column in the pi matrix
      number_of_inclusions_across_all_cancers = sum(NumIncludedAndAvail(covariates_by_cancer, covariates_in_model, gamma_i)[-1]) # remove the inclusion indicator for the intercept, since it is outside SS framework
      sum_I_p = sum(I_p[-1]) # the number of betas contained in the model, excluding the intercept
      pi[i+1,] = rbeta(1, 1 + number_of_inclusions_across_all_cancers, 1 + sum_I_p - number_of_inclusions_across_all_cancers)
